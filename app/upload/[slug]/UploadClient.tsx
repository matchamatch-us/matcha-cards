...
export default function UploadClient({ slug }: { slug: string }) {
  const sp = useSearchParams();

  // This is the key fix: read token from the URL in the client
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // keep comfortably under common Vercel payload limits
      if (outFile.size > 3_500_000) {
        setMsg(`That photo is still too large (${prettySize(outFile.size)}). Try a different photo or crop it.`);
        setBusy(false);
        return;
      }
      ...
      const res = await fetch(
        `/api/upload?slug=${encodeURIComponent(slug)}&token=${encodeURIComponent(token)}`,
        { method: "POST", body: fd }
      );
      ...
      // Follow redirect to /p/<slug> typically
      ...
      >
        ...
        {!token ? (
          ...
          >
            ...
            <div style={{ marginTop: 10, opacity: 0.65, fontSize: 14 }}>
              Debug: slug=<b>{slug}</b>, token=<b>{token ? "present" : "missing"}</b>
            </div>
          </div>
        ) : (
          <form
